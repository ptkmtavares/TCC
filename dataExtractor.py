...
def extract_emails(text: str) -> List[str]:
    if not text:
        return []
    in_brackets = re.findall(r'<([a-zA-Z0-9+._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)>', text)
    if not in_brackets:
        not_in_brackets = re.findall(r'([a-zA-Z0-9+.-]+@[a-zA-Z0-9.-]+\.[a-zA-Z0-9_-]+)', text)
...
def get_str_features(email_info: Dict[str, str], info_name: str, feature_list: Dict[str, int], features: List[str], conditions_check: List[str]) -> None:
    for feature_name, condition_check in zip(features, conditions_check):
        if condition_check == '':
            feature_list[feature_name] = 1 if re.match(condition_check, email_info[info_name]) else 0
        else:
            feature_list[feature_name] = 1 if re.search(condition_check, email_info[info_name], re.IGNORECASE) else 0
...
def get_missing_features(email_info: Dict[str, str], feature_list: Dict[str, int]) -> None:
...
def get_features_array(email_info: Dict[str, str]) -> List[int]:
    ...
    conditions_check = ['']
    get_str_features(email_info, info_name, features_dict, features_names, conditions_check)
    info_name = 'from'
    ...
    features_dict['email_match_from_reply-to'] = email_same_check(from_emails, reply_to_emails)
    ...
    features_dict['domain_match_message-id_from'] = domain_match_check(message_id_domains, from_domains)
    features_dict['domain_match_from_return-path'] = domain_match_check(from_domains, return_path_domains)
    features_dict['domain_match_message-id_return-path'] = domain_match_check(message_id_domains, return_path_domains)
    features_dict['domain_match_message-id_sender'] = domain_match_check(message_id_domains, sender_domains)
    features_dict['domain_match_message-id_reply-to'] = domain_match_check(message_id_domains, reply_to_domains)
    features_dict['domain_match_return-path_reply-to'] = domain_match_check(return_path_domains, reply_to_domains)
    features_dict['domain_match_reply-to_to'] = domain_match_check(reply_to_domains, to_domains)
    features_dict['domain_match_to_in-reply-to'] = domain_match_check(to_domains, in_reply_to_domains)
    features_dict['domain_match_errors-to_message-id'] = domain_match_check(errors_to_domains, message_id_domains)
    features_dict['domain_match_errors-to_from'] = domain_match_check(errors_to_domains, from_domains)
    features_dict['domain_match_errors-to_sender'] = domain_match_check(errors_to_domains, sender_domains)
    features_dict['domain_match_errors-to_reply-to'] = domain_match_check(errors_to_domains, reply_to_domains)
    features_dict['domain_match_sender_from'] = domain_match_check(sender_domains, from_domains)
    features_dict['domain_match_references_reply-to'] = domain_match_check(references_domains, reply_to_domains)
    features_dict['domain_match_references_in-reply-to'] = domain_match_check(references_domains, in_reply_to_domains)
    features_dict['domain_match_references_to'] = domain_match_check(references_domains, to_domains)
...
def process_email(line: str, email_cache: Dict[str, List[int]], label_dict: Dict[str, int]) -> Tuple[Union[List[int], None], Union[int, None], Union[str, None]]:
...
def get_training_test_set(index_path: str, values: List[str], percent: float) -> Tuple[List[List[int]], List[int]]:
...
def get_example_test_set(index_path: str) -> Tuple[List[List[int]], List[int], List[str]]:
    with open(index_path, 'r', encoding='latin_1') as f:
        lines = f.readlines()
