...
def train_mlp_tune(config: Dict[str, Any], train_data: torch.Tensor, train_labels: torch.Tensor, test_data: torch.Tensor, test_labels: torch.Tensor, example_data: torch.Tensor, example_labels: torch.Tensor) -> None:
    """
    Função para treinar o MLP com Ray Tune.
    """
    try:
        ...
        model = MLP(input_dim, hidden_dim1, hidden_dim2, output_dim, l1_lambda=config["l1_lambda"], l2_lambda=config["l2_lambda"], dropout=config["dropout"]).to(DEVICE)
        criterion = nn.CrossEntropyLoss()
        optimizer = optim.Adam(model.parameters(), lr=config["lr"], weight_decay=config["weight_decay"])

        train_mlp(model, criterion, optimizer, X_train, y_train, X_test, y_test, num_epochs=config["num_epochs"], patience=config["patience"], printInfo=False)
...
def trial_dirname_creator(trial: Trial) -> str:
    """
    Função para criar o nome do diretório do trial.
    """
    return f"trial_{trial.trial_id}"

def get_hyperparameters(train_set: np.ndarray, test_set: np.ndarray, train_labels: np.ndarray, test_labels: np.ndarray, example_data: List[List[int]], example_labels: List[int], config: str = None) -> Dict[str, Any]:
    """
    Função para obter os melhores hiperparâmetros usando Ray Tune.
    """
    if config == 'phishing':
        config = {
    ...
    try:
        analysis = tune.run(
            tune.with_parameters(train_mlp_tune, train_data=train_set, train_labels=train_labels, test_data=test_set, test_labels=test_labels, example_data=example_data, example_labels=example_labels),
